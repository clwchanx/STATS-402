...
summary(m_ind)
```
Then we do the additive log transform model fitted on the variable salary.

```{r}
model.a1 = aov(log10(salary_in_usd) ~ ., data = df)
summary(model.a1)

```
So after fitting the model with the log-transformed salary, it turns out that all the predictors—work year, experience level, job title, remote ratio, company location, and company size—are significant, meaning they all have a real impact on salary. Experience level and company location stand out as the biggest drivers, with the highest F-values, suggesting they influence salary the most. Job title and remote ratio also matter quite a bit

```{r}
residuals.log = residuals(model.a1)

plot(residuals.log, main = "residuals after log transformation", 
     ylab = "residuals", xlab = "index", col = 'yellow')
abline(h = 0, col = "red", lwd = 2)

```


The log-transformed additive model is good but not perfect. The log transformation did help a bit where most residuals are centered around zero, which is a good sign. However, there’s still a slight trend and the spread of the residuals isn’t quite even.
