...
```{r}
df %>% 
  group_by(experience_level,company_location) %>% 
  summarize(salaries = mean(salary_in_usd)) -> tips2


tips2 %>% 
  ggplot() +
  aes(x = experience_level, y = salaries, color = company_location) +
  geom_line(aes(group = company_location)) +
  geom_point()+
  labs(title = "Interaction Plot Between Experience Level and Company Location")
```
